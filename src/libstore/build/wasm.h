// Copied from https://github.com/WebAssembly/wasm-c-api/blob/2ce1367c9d1271c83fb63bef26d896a2f290cd23/include/wasm.h
// We add this because we don't link to it normally.
#define WASM_API_EXTERN typedef
// Because of this we can't use any of the helper functions unfortunately,
// since the functions they use aren't defined.
// original license: (Apache 2.0) https://github.com/WebAssembly/wasm-c-api/blob/2ce1367c9d1271c83fb63bef26d896a2f290cd23/LICENSE
// WebAssembly C API
...
static inline void wasm_name_new_from_string(
  wasm_name_new* wasm_name_new_,
  own wasm_name_t* out, const char* s
) {
  wasm_name_new_(out, strlen(s), s);
}

static inline void wasm_name_new_from_string_nt(
  wasm_name_new* wasm_name_new,
  own wasm_name_t* out, const char* s
) {
...
static inline bool wasm_valtype_is_num(
  wasm_valtype_kind* wasm_valtype_kind_,
  const wasm_valtype_t* t
) {
  return wasm_valkind_is_num(wasm_valtype_kind_(t));
}

static inline bool wasm_valtype_is_ref(
  wasm_valtype_kind* wasm_valtype_kind_,
  const wasm_valtype_t* t
) {
  return wasm_valkind_is_ref(wasm_valtype_kind_(t));
}
...
// wasmtime extensions, copied from wasmtime headers, not originally
// part of wasm-c-api's wasm.h
